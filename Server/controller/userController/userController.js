...
const createUser = async (req, res) => {
  try {
    ...
    axios.post(awsEmailResisterUrl, { InstructorEmail: user.email })
      .then(response => {
    ...
    if (e.code === 11000) {
      ...
      return res.status(400).send({ error: `The ${field} '${value}' is already in use.` });
    }
...
const login = async (req, res) => {
  try {
    ...
    res.status(400).send()
    console.log(e, "iam error in usercontroller")
  }
}
